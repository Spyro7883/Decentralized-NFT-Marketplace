...
contract NftMarketplace is ReentrancyGuard {
     ...
     * @param tokenId: sale price of the listed NFT
        ...
        isOwner(nftAddress, tokenId, msg.sender)
    {
        ...
        // 1. Send the NFT to the contract. Transfer -> Contrag "hold" the NFT.
    ...
    function cancelListing(address nftAddress, uint56 tokenId)
        external
        isOwner(nftAddress, tokenId, msg.sender)
        isListed(nftAddress, tokenId)
    {
...
/*  1. `listItem`: List NFTs on the marketplace X
    2. `buyItem`: Buy the NFTs X
    3. `cancelItem`: Cancel a listing X
    4. `updateListing`: Update Price X
    5. `withdrawProceeds`: Withdraw payment for any bought NFTs X
*/
